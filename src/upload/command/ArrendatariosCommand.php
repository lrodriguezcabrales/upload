...
class ArrendatariosCommand extends Command
{	
...
// 	public $serverRoot = 'http:/104.130.12.152/';
    ...
    function tipoArrendatario($tipo) {
    	
    	$leaseType = null;
    	
    	//Cooarrendatario
    	if($tipo == 'C'){
    		$leaseType = array('id'=>'33645952-c4f1-47d4-b64b-30cdf60c6d5e');
    	}
    	//Arrendatario
    	if($tipo == 'P'){
    		$leaseType = array('id'=>'3b13acd5-41ef-4446-8ddb-de47a85597b6');
    	}
...
//     	print_r($bank);
    ...
    function searchTipoContratoMandato($conArriendo) {
    	
    	$typeContract = null;

    	if($conArriendo['por_seguro'] > 0){
    		//Con garantia
...
//     	if($conArriendo['aseg'] = 0){
//     		$typeContract = array('id' => '54530e43-abfc-44e2-a98f-b9fcf51003fa');
//     	}
    	 
//     	//Arriendo con garantia
//     	if($conArriendo['aseg'] = 1){
//     		$typeContract = array('id' => '53eee515-a699-498b-a65f-16336660c350');
//     	}
    ...
    function searchConArriendoSF2($conArriendo) {
    	
    	
    	$filter = array(
    			'value' => $this->cleanString($conArriendo['id_ConArriendo']),
    			'operator' => '=',
    			'property' => 'consecutive'
    	);
    	$filter = json_encode(array($filter));
    ...
    function searchMandateContract($conArriendo) {
    	 
    	$filter = array(
    			'value' => $this->cleanString($conArriendo['id_ConArriendo']),
    			'operator' => '=',
    			'property' => 'ConArriendoSifincaOne'
    	);
    	$filter = json_encode(array($filter));
    ...
    function searchMandateContractPorConArriendoEinmueble($conArriendo) {
    	...
    	$filter[] = array(
    			'value' => $this->cleanString($conArriendo['id_inmueble']),
    			'operator' => '=',
    ...
    function searchMandateContractByProperty($conArriendo) {
    ...
    function searchUsuario($conArriendo) {
    
    	$usuarioSF1 = $conArriendo['emailCatcher'];
    	...
    	$filter = array(
    			'value' => $this->cleanString($conArriendo['emailCatcher']),
    			'operator' => '=',
    			'property' => 'email'
    	);
    	$filter = json_encode(array($filter));
    ...
    function searchOwner($conArriendo) {
    	
    	$codigoConArriendo = $this->cleanString($conArriendo['id_ConArriendo']);
    	
    	$filter = array(
    			'value' => $codigoConArriendo,
    			'operator' => 'equal',
    ...
    function searchArrendatarioPorConArriendoYcliente($client, $conArriendo) {
    	...
    	$filter[] = array(
    			'value' => $codigoConArriendo,
    			'operator' => 'equal',
    			'property' => 'leasingContract.consecutive'
    	);
    	...
    	$urlClientSF2 = $this->server.'catchment/main/lease?filter='.$filter;
    	//echo "\n".$urlClientSF2."\n";
