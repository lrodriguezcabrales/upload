...
class FotosInmueblesBogotaCommand extends Command
{	
    ...
    {
        $this->setName('fotosBogota')
		             ->setDescription('Obtener y guardar fotos inmuebles - Bogota');
	}
    ...
    function buildFotos($inmueblesCtg) {
    	...
    	$inmueblesSF2 = $inmueblesCtg->getInmueblesFotosBogota();
    	...
    	if($totalInmueblesSF2 > 0){
    		...
    		//foreach ($inmueblesSF2 as $inmueble) {
    				...
    				if(!is_null($property)){
    					...
    					$urlFotoSF1 = 'http://10.102.1.6/ws-img/foto.php?key=B';
    					...
    					if(count($fotos) > 0){
    						...
    						for ($i = 0; $i < count($fotos); $i++) {
    							...
    							if(!$existPhoto){
									...
									$path = "/var/www/html/upload3/fotosinmuebleBogota/".$foto['id']."/";
