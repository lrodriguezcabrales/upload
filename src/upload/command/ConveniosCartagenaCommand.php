...
// 	public $serverRoot = 'http:/104.130.12.152/';
    ...
    public function buildOwner($conexion, $convenio) {
    	...
    	$ownerIdentificacion = $convenio['id_cliente'];
    	$ownerIdentificacion = $this->cleanString($ownerIdentificacion);
//     	echo "\nownerIdentificacion\n";
//     	echo $ownerIdentificacion;
    	...
    	//for ($i = 5; $i < $totalOwners; $i++) {
    		...
    		if($client){
    			...
    			$ownerType = $this->tipoPropietario($convenio['tipo_relacion']);
    			...
    			$bOwner = array(
    					...
    					'ownerType' => $ownerType,
    					'percentageIncomeDivision' => $convenio['participacion'], //% Participacion de renta
...
//     	}
    ...
    function buildConvenio($conexion, $convenios) {
    	...
    	for ($i = 20000; $i < $totalConvenios; $i++) {
    		...
    		if(is_null($convenioSF2)){
    			...
    			if(isset($convenio['id_cliente'])){
    				...
    				//$mandateContract = $this->buildContractoDeMandato($convenio);
